...
import {useClientsStaffDeleteDestroyMutation} from '@/services/IsyBuildApi';
...
const DeleteUserClientContent = ({handleClose, handleCloseWithoutRefresh, id}: DeleteProps) => {
  const [deleteUser, {isLoading}] = useClientsStaffDeleteDestroyMutation();
  const {setOpenSnackBar, setInfoAlert} = useContext(SnackBarContext) as SnackBarContextType;

  const handleDelete = async () => {
    try {
      await deleteUser({clientStaffId: id}).unwrap(); // Passez l'identifiant de l'utilisateur à la mutation
      handleClose();
...
export default DeleteUserClientContent;
