    ...
    <Card>
      <CardContent className='flex flex-col gap-6'>
        ...
        <div className='flex items-center gap-3'>
          {getAvatar({
            avatar: categoryData?.created_by?.avatar ?? '/images/avatars/1.png',
            email: categoryData?.created_by.email ?? ''
          })}
          <div className='flex flex-col'>
            <Typography color='text.primary' className='font-medium'>
              {categoryData?.created_by.first_name} {categoryData?.created_by.last_name}
            </Typography>
              <Typography>{categoryData?.created_by.email}</Typography>
          </div>
        </div>
          ...
          {categoryData ? (
            <>
              <div className='flex items-center gap-2 mlb-2'>
                ...
                <Typography> {formatDate(categoryData.created_at)}</Typography>
              </div>
              <div className='flex items-center gap-2 mlb-2'>
                <i className='tabler-clock-hour-4' />
                <Typography>Mis à jour à:</Typography>
                <Typography> {formatDate(categoryData.updated_at)}</Typography>
              </div>
            </>
          ) : (
