...
const DocDiffEdit: React.FC = () => {
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<FormValidateDocDiffUpdateType>({
  ...
  const [updateDocDiff, { isLoading: isUpdating }] =
    useDocumentDiffusionUpdateMutation();
  ...
  useEffect(() => {
    if (docDiffData) {
      ...
      setDocType(docDiffData.type); // Set the document type
    }
  ...
  const onSubmit = async (data: FormValidateDocDiffUpdateType) => {
    try {
      ...
      setInfoAlert({
        severity: "error",
        message: "Échec de la modification du document",
      });
    }
  };
          ...
          <Grid container spacing={6}>
            <Grid item xs={12}>
              <DocDiffInformation
                register={register}
                errors={errors}
                phaseValue={docDiffData?.phase}
              />
            </Grid>
            {docType !== "fiche_technique_produit" && (
              <Grid item xs={12}>
                <DocumentDocDiff
                  DocumentDocDiffData={DocumentDocDiffData}
