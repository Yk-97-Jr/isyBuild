...
import DiffuseAddDialog from "../config/DiffuseAddDialog";
...
type RowSelection = {
  [key: string]: boolean; // Keys are row IDs, and values indicate selection state
};
...
const DocDiffTable = ({
  ...
  const handleEditDocument = (id: number) => {
    ...
    router.push(
      `/${userRole}/projects/${edit}/details/documentDiffusions/${id}/details`,
    );
  };
  ...
  useEffect(() => {
    ...
    const selectedRowIds = selectedRowIndices
      .map((index) => {
        const row = data ? data[index] : undefined;

        return row?.id || null;
      })
      .filter(Boolean); // Remove nulls

    setSelectedIds(selectedRowIds as number[]);
  }, [rowSelection, data]);
  ...
  const columns = useMemo<ColumnDef<LocalisationWithAction, any>[]>(
    () => [
          ...
          <Checkbox
            {...{
              ...
              onChange: (e) => {
                ...
                // Use row.original.<field> for unique identifier
                const id = row.original.id; // Replace `id` with your unique field if different
      ...
      columnHelper.accessor("type", {
        header: "Type ",
        cell: ({ row }) => {
          const { label, color } = getStatusProps<Type94BEnum>(
            row.original.type,
            Type474Mapping,
          );
      ...
      columnHelper.accessor("phase", {
        ...
        cell: ({ row }) => {
          ...
          const { label, color } = getStatusProps<PhaseEnum>(
            row.original.phase,
            PhaseStatusMapping,
          );
      ...
      columnHelper.accessor("diffusion_date", {
        ...
        cell: ({ row }) => (
          <Typography>{formatDate(row.original.diffusion_date)}</Typography>
        ),
      }),
      ...
      <Card>
        <div className="flex justify-between flex-col items-start md:flex-row md:items-center p-6 border-bs gap-4">
          <div className="flex flex-col sm:flex-row max-sm:is-full items-start sm:items-center gap-4">
            {selectedIds.length > 0 && ( // Show button only if there are selected rows
              <OpenFinanceOnElementClick
                element={Button}
                elementProps={buttonProp}
                dialog={DiffuseAddDialog}
                dialogProps={{ refetch, selectedIds }} // Pass selected IDs to dialogProps
              />
            )}
            <CustomTextField 
              select
