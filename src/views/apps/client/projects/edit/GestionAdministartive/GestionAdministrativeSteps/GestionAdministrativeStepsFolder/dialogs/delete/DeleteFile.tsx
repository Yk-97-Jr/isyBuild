...
import {useDeleteSuiviAdministrativeStepDocumentMutation} from "@/services/IsyBuildApi";
...
const DeleteFile = ({open, setOpen, id, refetch}: Props) => {
  ...
  const [deleteSuiviDocumentProject, {isLoading}] = useDeleteSuiviAdministrativeStepDocumentMutation();
  ...
  const handleDelete = async () => {
    ...
    try {
      if (id !== undefined) {
        try {
          await deleteSuiviDocumentProject({stepDocumentId: +id}).unwrap();
