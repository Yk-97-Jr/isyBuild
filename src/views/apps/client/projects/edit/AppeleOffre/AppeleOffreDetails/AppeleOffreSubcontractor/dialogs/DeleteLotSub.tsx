...
const DeleteLotSub = ({open, setOpen, id, refetch}: AddProps) => {
  const {setOpenSnackBar, setInfoAlert} = useContext(SnackBarContext) as SnackBarContextType;
  const [deleteSubLotProject, {isLoading}] = useProjectLotsSubcontractorsRemoveDestroyMutation();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSubLotProject({projectLotSubcontractorId: +id}).unwrap() // Passez l'identifiant de l'utilisateur à la mutation


      handleClose();
      setOpenSnackBar(true);
      setInfoAlert({severity: 'success', message: 'Entreprise assinger etait supprimer avec succès'});
    } catch (error) {
  ...
  return (
    <Dialog open={open} onClose={handleCloseWithoutRefresh}
      ...
      <DialogActions
        ...
        >
          {isLoading ? <CircularProgress sx={{color: 'white'}} size={24}/> : "Supprimer Assignement"}
        </Button>
